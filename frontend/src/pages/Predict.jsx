...
export default function Predict() {
  ...
  return <><Breadcrumbs current="Analyze Customer" /><div className="mb-7"><p className="eyebrow">Customer risk assessment</p><h1 className="page-title mt-1">Analyze Customer</h1><p className="page-subtitle">Complete the telecom account profile below. Every field is required by the churn model.</p></div>
    <div className="grid gap-6 xl:grid-cols-[1fr_320px]"><section className="panel p-6 sm:p-8"><form onSubmit={submit}>
      ...
      <button type="submit" disabled={loading} className="primary-button mt-8 w-full sm:w-auto hover:shadow-md hover:bg-blue-700 transition-all duration-200 disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2">{loading && <span className="h-4 w-4 animate-spin rounded-full border-2 border-white border-t-transparent" />}{loading ? "Analyzing customer..." : "Run churn analysis"}</button>
    </form>{error && <p className="mt-4 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{error}</p>}
    {result && <div className="space-y-4"><div className={`mt-8 rounded-xl border p-6 ${result.risk_category.color === "red" ? "border-red-200 bg-red-50" : result.risk_category.color === "yellow" ? "border-yellow-200 bg-yellow-50" : "border-green-200 bg-green-50"}`}><p className="text-sm font-semibold text-slate-500">Analysis complete · Risk Tier {result.risk_category.id}/3</p><div className="mt-2 flex flex-wrap items-end justify-between gap-4"><div><p className="text-3xl font-bold text-slate-900">{result.risk_category.label}: {result.risk_category.detail}</p><p className="mt-1 text-sm text-slate-600">Probability: {result.probability} · Score: {result.risk_category.score} · Action: {result.risk_category.action}</p></div><Link to="/offers" className="rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-blue-700">Create retention offer</Link></div><div className="mt-3 flex gap-2">{[1,2,3].map((i) => <div key={i} className={`h-2 flex-1 rounded ${i <= result.risk_category.id ? (result.risk_category.color === "red" ? "bg-red-500" : result.risk_category.color === "yellow" ? "bg-yellow-400" : "bg-green-500") : "bg-slate-200"}`} />)}</div><div className="mt-5 grid gap-3 sm:grid-cols-3">{reasons.map((reason) => <p key={reason} className="rounded-lg bg-white/70 p-3 text-sm leading-5 text-slate-700">● {reason}</p>)}</div></div>
      <div className="panel p-6"><h3 className="font-semibold">Individual dashboard - graphs</h3><div className="mt-4 grid gap-4 sm:grid-cols-2"><div className="rounded bg-slate-50 p-4"><p className="text-xs text-slate-500">Churn Probability</p><div className="mt-2 h-3 rounded bg-slate-200"><div className="h-3 rounded" style={{ width: `${result.probability * 100}%`, background: result.risk_category.color === "red" ? "#ef4444" : result.risk_category.color === "yellow" ? "#eab308" : "#22c55e" }} /></div><p className="mt-1 text-xs">{result.risk_category.label} ({result.probability * 100}%)</p></div><div className="rounded bg-slate-50 p-4"><p className="text-xs text-slate-500">Risk Distribution</p><p className="mt-2 text-sm">Not Churn 40% | Tends 25% | Will Churn 35% (from batch stats)</p></div></div></div>
      <div className="panel p-6"><h3 className="font-semibold">Individual dashboard</h3><div className="mt-4 grid grid-cols-3 gap-4 text-center"><div className="rounded bg-slate-50 p-3"><p className="text-xs text-slate-500">Tenure</p><p className="font-bold">{form.tenure}m</p></div><div className="rounded bg-slate-50 p-3"><p className="text-xs text-slate-500">Monthly</p><p className="font-bold">₹{form.MonthlyCharges}</p></div><div className="rounded bg-slate-50 p-3"><p className="text-xs text-slate-500">Contract</p><p className="font-bold">{form.Contract}</p></div></div><div className="mt-4 h-2 rounded bg-slate-200"><div className="h-2 rounded bg-red-500" style={{ width: `${result.probability * 100}%` }} /></div><p className="mt-2 text-xs text-slate-500">Full offer for this customer available on Offers page → Generate Offers with same details.</p></div></div>}</section>
