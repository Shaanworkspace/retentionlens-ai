...
export default function Predict() {
  ...
  return <><Breadcrumbs current="Analyze Customer" /><div className="mb-7"><p className="eyebrow">Customer risk assessment</p><h1 className="page-title mt-1">Analyze Customer</h1><p className="page-subtitle">Complete the telecom account profile below. Every field is required by the churn model.</p></div>
    ...
    {result && <div className="space-y-4"><div className={`mt-8 rounded-xl border p-6 ${result.risk_category.color === "red" ? "border-red-200 bg-red-50" : result.risk_category.color === "orange" ? "border-orange-200 bg-orange-50" : result.risk_category.color === "yellow" ? "border-yellow-200 bg-yellow-50" : "border-green-200 bg-green-50"}`}><p className="text-sm font-semibold text-slate-500">Analysis complete · Risk Tier {result.risk_category.id}/5</p><div className="mt-2 flex flex-wrap items-end justify-between gap-4"><div><p className="text-3xl font-bold text-slate-900">{result.risk_category.label}: {result.risk_category.detail}</p><p className="mt-1 text-sm text-slate-600">Probability: {result.probability} · Score: {result.risk_category.score} · Action: {result.risk_category.action}</p></div><Link to="/offers" className="rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-blue-700">Create retention offer</Link></div><div className="mt-3 flex gap-2">{[1,2,3,4,5].map((i) => <div key={i} className={`h-2 flex-1 rounded ${i <= result.risk_category.id ? (result.risk_category.color === "red" ? "bg-red-500" : result.risk_category.color === "orange" ? "bg-orange-500" : result.risk_category.color === "yellow" ? "bg-yellow-400" : "bg-green-500") : "bg-slate-200"}`} />)}</div><div className="mt-5 grid gap-3 sm:grid-cols-3">{reasons.map((reason) => <p key={reason} className="rounded-lg bg-white/70 p-3 text-sm leading-5 text-slate-700">● {reason}</p>)}</div></div>
