...
export default function Offers() {
  ...
  return (
    <main className="mx-auto min-h-[calc(100vh-100px)] max-w-6xl px-5 py-10 sm:px-8">
      ...
      {data && <div className="mt-6"><p className="text-sm text-slate-500">Churn assessment</p><p className="mt-1 font-semibold text-slate-900">{data.churn_label} <span className="font-normal text-slate-600">({data.probability})</span></p><p className="mt-6 text-sm font-semibold text-slate-900">Recommended offers</p><div className="mt-2 grid gap-3">{(data.offers && Array.isArray(data.offers) ? data.offers : [data.offers_text || data.offers]).map((offer, i) => <div key={i} className="rounded-xl border border-slate-200 bg-slate-50 p-4"><p className="text-sm font-bold">Offer {i + 1}</p><p className="mt-1 text-sm leading-6 text-slate-700">{offer}</p></div>)}</div></div>}
      </div>
